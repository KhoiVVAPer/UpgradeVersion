...
RCT_EXPORT_METHOD(convert:(NSDictionary *)options
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSString *cachedHTMLPath = [paths objectAtIndex:0];
        NSString *cachedHTMLString = [NSString stringWithFormat:@"%s/%@/%s", "file:/", cachedHTMLPath, "testHTML.html"];
        NSMutableURLRequest *cachedHTMLURL = [NSURL URLWithString:cachedHTMLString];

        NSString *HTML = _html;
        NSData *data = [HTML dataUsingEncoding: NSUTF8StringEncoding];
        [data writeToURL: cachedHTMLURL atomically: YES];
        // [_webView loadRequest: [NSURLRequest requestWithURL: cachedHTMLURL]];
        [_webView loadFileURL:cachedHTMLURL allowingReadAccessToURL:cachedHTMLURL];
    });
