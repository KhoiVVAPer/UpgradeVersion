...
cp -p GoogleService-Info-prod.plist GoogleService-Info.plist
cp -p google-services.json-prod android/app/google-services.json

sed -i.bak 's/"bundleId": "'"$CURRENT_BUNDLE_ID"'"/"bundleId": \"'"$PROD_BUNDLE_ID"'"/g' package.json
sed -i.bak 's/'"$STAGE_BUNDLE_ID"';/'"$PROD_BUNDLE_ID"';/g' ios/AK_DCA_App.xcodeproj/project.pbxproj
