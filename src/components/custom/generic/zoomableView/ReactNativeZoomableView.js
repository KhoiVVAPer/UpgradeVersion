...
class ReactNativeZoomableView extends Component {
  ...
  _handleMoveShouldSetPanResponder = (e, gestureState) => {
    ...
    if(Platform.OS === 'ios'){
      if((zoomLevel <= 1 && gestureState.numberActiveTouches < 2) ||
      ((this._getZoomableViewEventObject().distanceBottom > 0 && gestureState.numberActiveTouches < 2 &&  this.lastYMove !== gestureState.moveY) && (!this._isValidHorizontalSwipe(gestureState) && gestureState.numberActiveTouches < 2)) ||
       ((this._getZoomableViewEventObject().distanceTop > 0 && gestureState.numberActiveTouches < 2 &&  this.lastYMove !== gestureState.moveY) && (!this._isValidHorizontalSwipe(gestureState) && gestureState.numberActiveTouches < 2))) {
       return false;
     }
    }

    this.lastYMove = gestureState.moveY;
