...
const productListPdf = (productListData, pageId, structureData, toReturnStr) => new Promise(async (resolve) => {
  const fontUrlArr = await fontToBase64();
  const breadcrumbData = await getBreadcrumbArr(pageId, structureData);
  htmlStr = '';
  htmlStr += ribbonData(productListData);
  htmlStr += await productList(productListData.productListArr);
